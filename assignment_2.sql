...
SQL> CREATE PLUGGABLE DATABASE pdb1
  2     ADMIN USER pdbadmin IDENTIFIED BY Darryl;

Pluggable database created.

-- Create another pluggable database named plsql_class2024db with admin user
SQL> create pluggable database plsql_class2024db admin user pdbadmin identified by Darryl;
...
SQL> CREATE USER da_plsqlauca IDENTIFIED BY Darryl;

User created.

-- Grant all privileges to the newly created user
SQL> GRANT all privileges to da_plsqlauca;
...
-- Create a new pluggable database named da_to_delete_pdb with admin user
SQL> CREATE PLUGGABLE DATABASE da_to_delete_pdb
  2     ADMIN USER pdbadmin IDENTIFIED BY Darryl;

Pluggable database created. -- The pluggable database is created

-- Drop the pluggable database da_to_delete_pdb including its datafiles
SQL> DROP PLUGGABLE DATABASE da_to_delete_pdb INCLUDING DATAFILES;
